...
class MGB_BridgeRunner {
	...
	public function render($data=null,$production=false){
		...
		/* Check for Multi Assets first */
		$customCollectibleCount = 0;
		if( isset( $data['imageData'][ 'collectibles' ]['group'] ) && is_array( $data['imageData'][ 'collectibles' ]['group'] ) ) {
			
			unset($this->assets['collectible']);
			unset($this->assets['collectibleList']);
			//print_r($data['imageData'][ 'collectibles' ]);
			foreach( $data['imageData'][ 'collectibles' ]['group'] as $k => $collectible ) {
				echo 'this.game.load.image("collectible_'.$data['id'].'_'.$k.'", "'.$collectible['url'].'");'."\n";
				$customCollectibleCount++;
			}
			echo 'this.game.playData["'.$data['id'].'"].customCollectibles = '.$customCollectibleCount.';'."\n";
		}
		//ground 
		$platformCount = 0;
		if( isset( $data['imageData'][ 'platforms' ]['group'] ) && is_array( $data['imageData'][ 'platforms' ]['group'] ) ) {
			
			unset($this->assets['shortFloor1']);
			unset($this->assets['shortFloor2']);
			unset($this->assets['mediumFloor']);
			unset($this->assets['longFloor']);
			//print_r($data['imageData'][ 'collectibles' ]);
			foreach( $data['imageData'][ 'platforms' ]['group'] as $k => $platform ) {
				echo 'this.game.load.image("platform_'.$data['id'].'_'.$k.'", "'.$platform['url'].'");'."\n";
				$platformCount++;
			}
			echo 'this.game.playData["'.$data['id'].'"].customPlatforms = '.$platformCount.';'."\n";
		}
